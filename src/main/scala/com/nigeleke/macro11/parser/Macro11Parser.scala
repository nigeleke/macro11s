...
//    ??? // singleOperandInstruction | doubleOperandInstruction | branchInstruction | sobInstruction
//
//  def instructionLine: Parser[InstructionLine] =
//    (labels ~ opt(instructionParser) ~ opt(comment)) ^^ { case ls ~ i ~ c => InstructionLine(ls, i, c) }
//
//  def directive: Parser[Directive]         = ???
//  def directiveLine: Parser[DirectiveLine] = directive ^^ { DirectiveLine.apply }
//
//  def statementLine: Parser[Statement] = instructionLine ||| directiveLine
//  def program: Parser[Program]             = rep(statementLine) ^^ { Program.apply }
//
//object Macro11Parser extends Macro11Parser:
//  def main(args: Array[String]) =
//    val result = Macro11Parser.parse(instructionParser, "DECB R0")
//    println(result)
